                    ...
                    if ($curObj.is("select")) {
                        ...
                        if (_ffIsMobile){
							...
							function rgb2hex(rgb){
								...
								("0" + (parseInt(rgb[1],10)-70).toString(16)).slice(-2) +
								("0" + (parseInt(rgb[2],10)-70).toString(16)).slice(-2) +
								("0" + (parseInt(rgb[3],10)-70).toString(16)).slice(-2) : '';
							}
                            ...
                            $menuElement.on("click","LI",function(){
                                ...
                                if (!curOpt.hasClass("selected") && !curOpt.hasClass("disabled")) {
									...
									if($('.egw_icon_btn').length){
										$('.egw_btn_wrp .egw_icon_btn').each(function() {
											$(this).removeClass().removeAttr('style').removeAttr('class');
											var bg=$(this).parents('fieldset ').find('.wp-color-result').css('background-color');
											$(this).addClass('egw_icon_btn ' + $curVal).css('background-color',bg).css('outline-color',bg);
										});
										$('.btn_effect .egw_icon_btn').each(function() {
											var str = $(this).attr('class');
											var res = str.split(" ");
											$(this).removeAttr('class');
											$(this).addClass('egw_icon_btn '+res[1]+' '+ $curVal);
										});
									}
					
                                        var curInd = $("LI", $menuElement).not(".ffGroup").index(curOpt);
                                        $curObj.val($curVal);
                                        var $curSpan = $("span", $dummyButton).prop("class",curOpt.prop("class")).text($curText);
